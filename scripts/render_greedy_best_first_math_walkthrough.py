#!/usr/bin/env python3
"""Render a Shorts-ready Greedy Best-First math walkthrough."""
...
DEFAULT_OUTPUT = SHORTS_DIR / "greedy_best_first_math_walkthrough.mp4"
DEFAULT_THUMBNAIL = SHORTS_DIR / "greedy_best_first_math_walkthrough_thumbnail.png"
...
def build_iterations(count: int = DEFAULT_EXPANSIONS) -> tuple[Iteration, ...]:
    ...
    counter = 0
    heapq.heappush(frontier, (heuristic(START), counter, counter, START))
    frontier_scores[START] = (heuristic(START), counter, counter)
    iterations: list[Iteration] = []

    for index in range(count):
        while frontier:
            h_score, tie_order, insert_order, current = heapq.heappop(frontier)
            if frontier_scores.get(current) == (h_score, tie_order, insert_order):
                break
        else:
        ...
        for neighbor in neighbors(current):
            ...
            previous_h = previous[0] if previous else None
            if previous is None or h_score < previous[0]:
                counter += 1
                g_score[neighbor] = tentative_g
                frontier_scores[neighbor] = (h_score, counter, counter)
                heapq.heappush(frontier, (h_score, counter, counter, neighbor))
            candidate_rows.append(Candidate(neighbor, "open", tentative_g, h_score, h_score, previous_h))
...
def draw_formula(draw: ImageDraw.ImageDraw, x: int, y: int, width: int) -> None:
    ...
    parts = (("priority", TEXT_YELLOW), (" = ", bubble.TEXT), ("h", TEXT_GREEN))
    cursor = x
    for text, color in parts:
        draw.text((cursor, y), text, font=font, fill=color)
        cursor += bubble.text_width(draw, text, font)
    draw.text((x, y + 62), "h: Manhattan estimate to G", font=small, fill=TEXT_GREEN)
    draw.text((x, y + 100), "g is tracked, but not used for priority", font=small, fill=TEXT_BLUE)
    draw.text((x, y + 138), "Greedy Best-First expands the smallest h.", font=small, fill=bubble.MUTED)
    ...
    *,
    ...
    for index, candidate in enumerate(iteration.candidates):
        ...
        if candidate.status == "wall":
            ...
            row, col = candidate.cell
            draw.text((x0 + 16, y + 58), f"h = |{row}-{goal_row}|+|{col}-{goal_col}|", font=line_font, fill=TEXT_GREEN)
            draw.text((x0 + 16, y + 96), f"  = {candidate.h}", font=line_font, fill=TEXT_GREEN)
            draw.text((x0 + 16, y + 134), f"priority = {candidate.h}", font=value_font, fill=TEXT_YELLOW)
            draw.text((x0 + 16, y + 172), f"g = {candidate.g}", font=line_font, fill=TEXT_BLUE)
...
def draw_frontier_panel(
    ...
    *,
    ...
    if state.mode in {"intro", "focus", "formula"}:
        draw.text((margin + 20, y + 72), "score neighbors, then choose the smallest h", font=small_font, fill=bubble.MUTED)
        return

    entries = sorted(iteration.frontier_after.items(), key=lambda item: item[1])[:5]
    cursor = margin + 20
    entry_y = y + 72
    for cell, (h_score, _, _) in entries:
        text = f"{cell}: h{h_score}"
    ...
    if iteration.chosen_next is not None and iteration.chosen_score is not None and state.mode in {"choose", "outro"}:
        lowest_h = iteration.chosen_score[0]
        tie_count = sum(1 for score in iteration.frontier_after.values() if score[0] == lowest_h)
        if tie_count > 1:
            chosen = f"next: {iteration.chosen_next} by tie order"
        else:
            chosen = f"next: {iteration.chosen_next} has lowest h"
...
def draw_frame(width: int, height: int, iterations: tuple[Iteration, ...], state: FrameState, frame_number: int, total_frames: int) -> Image.Image:
    ...
    subtitle = f"{len(iterations)} expansions, choosing the smallest h."
    ...
    footer = f"frame {frame_number + 1}/{total_frames} | Greedy priority uses Manhattan h only"
