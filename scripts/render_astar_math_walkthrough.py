#!/usr/bin/env python3
"""Render a Shorts-ready A* math walkthrough."""
...
DEFAULT_OUTPUT = SHORTS_DIR / "a_star_math_walkthrough.mp4"
DEFAULT_THUMBNAIL = SHORTS_DIR / "a_star_math_walkthrough_thumbnail.png"
...
START = (5, 2)
GOAL = (5, 9)
WALLS = frozenset({(3, 4), (4, 4), (5, 4), (6, 4), (7, 4)})
...
def build_iterations(count: int = DEFAULT_EXPANSIONS) -> tuple[Iteration, ...]:
    ...
    counter = 0
    heapq.heappush(frontier, (heuristic(START), heuristic(START), counter, START))
    frontier_scores[START] = (heuristic(START), heuristic(START), counter)
    iterations: list[Iteration] = []

    for index in range(count):
        while frontier:
            f_score, h_score, insert_order, current = heapq.heappop(frontier)
            if frontier_scores.get(current) == (f_score, h_score, insert_order):
                break
        else:
        ...
        for neighbor in neighbors(current):
            ...
            previous_f = previous[0] if previous else None
            if previous is None or f_score < previous[0]:
                counter += 1
                g_score[neighbor] = tentative_g
                frontier_scores[neighbor] = (f_score, h_score, counter)
                heapq.heappush(frontier, (f_score, h_score, counter, neighbor))
            candidate_rows.append(Candidate(neighbor, "open", tentative_g, h_score, f_score, previous_f))
...
def draw_formula(draw: ImageDraw.ImageDraw, x: int, y: int, width: int) -> None:
    ...
    parts = (("f", TEXT_YELLOW), (" = ", bubble.TEXT), ("g", TEXT_BLUE), (" + ", bubble.TEXT), ("h", TEXT_GREEN))
    cursor = x
    for text, color in parts:
        draw.text((cursor, y), text, font=font, fill=color)
        cursor += bubble.text_width(draw, text, font)
    draw.text((x, y + 62), "g: steps already taken", font=small, fill=TEXT_BLUE)
    draw.text((x, y + 100), "h: Manhattan estimate to G", font=small, fill=TEXT_GREEN)
    draw.text((x, y + 138), "A* expands the frontier cell with the smallest f.", font=small, fill=bubble.MUTED)
    ...
    *,
    ...
    for index, candidate in enumerate(iteration.candidates):
        ...
        if candidate.status == "wall":
            ...
            draw.text((x0 + 16, y + 58), f"g = {iteration.current_g}+1 = {candidate.g}", font=line_font, fill=TEXT_BLUE)
            goal_row, goal_col = GOAL
            row, col = candidate.cell
            draw.text((x0 + 16, y + 96), f"h = |{row}-{goal_row}|+|{col}-{goal_col}|", font=line_font, fill=TEXT_GREEN)
            draw.text((x0 + 16, y + 134), f"  = {candidate.h}", font=line_font, fill=TEXT_GREEN)
            draw.text((x0 + 16, y + 172), f"f = {candidate.g}+{candidate.h} = {candidate.f}", font=value_font, fill=TEXT_YELLOW)
...
def draw_frontier_panel(
    ...
    *,
    ...
    if state.mode in {"intro", "focus", "formula"}:
        draw.text((margin + 20, y + 72), "score neighbors, then choose the smallest f", font=small_font, fill=bubble.MUTED)
        return

    entries = sorted(iteration.frontier_after.items(), key=lambda item: item[1])[:5]
    cursor = margin + 20
    entry_y = y + 72
    for cell, (f_score, h_score, _) in entries:
        text = f"{cell}: f{f_score}"
    ...
    if iteration.chosen_next is not None and iteration.chosen_score is not None and state.mode in {"choose", "outro"}:
        lowest_f = iteration.chosen_score[0]
        tie_count = sum(1 for score in iteration.frontier_after.values() if score[0] == lowest_f)
        if tie_count > 1:
            chosen = f"next: {iteration.chosen_next} by tie order"
        else:
            chosen = f"next: {iteration.chosen_next} has lowest f"
...
def draw_frame(width: int, height: int, iterations: tuple[Iteration, ...], state: FrameState, frame_number: int, total_frames: int) -> Image.Image:
    ...
    subtitle = f"{len(iterations)} expansions, scored one neighbor at a time."
    ...
    footer = f"frame {frame_number + 1}/{total_frames} | Manhattan heuristic, unit movement cost"
